              ...
              if (qs){
                ...
                for (var j=0;j<records.length;j++){
                  ...
                  for (var k=0;k<qs.length;k++){
                    ...
                    if (records[j][qs[k].name] && records[j][qs[k].name].indexOf(qs[k].value)>=0){
                      match=true;
                      break;
                    }
                    else
                    {
                      if (qs[k].name==='search'){
                        ...
                        for (var field in records[j]){
                          if (records[j].hasOwnProperty(field) && records[j][field].toString().toLowerCase().indexOf(qs[k].value)>=0)
                          {
                            match=true;
                            break;
                          }
