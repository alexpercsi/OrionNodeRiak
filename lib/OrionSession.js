...
global.OrionSession = SC.Object.extend({
   ...
   sessionCookieExpireDuration: 5, // duration in days
   
   timeOutDuration: 5, //(timeout in minutes) 15 minutes standard 
   ...
   */
   ...
   _timeOutDurationCache: 1, // to cache the calculation of timeOutDuration to milliseconds
   ...
   checkSession: function(user,sessionInfo,sessionKeyOnly){
      ...
      if(!sessionKeyOnly){
        ...
        var substr = sessionInfo.substring(sessionInfo.indexOf('OrionServer=')+'OrionServer='.length);
        receivedSessionKey = (substr.indexOf(';')>=0)?substr.substring(0, substr.indexOf(';')):substr;
        sys.puts('actual session key is '+receivedSessionKey);
        for (var x in this._loggedInUsers){
           if (this._loggedInUsers.hasOwnProperty(x)){
             if (this._loggedInUsers[x].sessionKeys)
             {
               var sesKeyIndex = this._loggedInUsers[x].sessionKeys.indexOf(receivedSessionKey);
                if(sesKeyIndex> -1){
                   var lastSeen = this._loggedInUsers[x].lastSeen[sesKeyIndex];
                   var now = new Date().getTime();
                   if((now - lastSeen) > timeout_in_ms){ // diff between lastseen and now too large?
                      // delete user key
                      this._loggedInUsers[x] = undefined;
                      return NO; // 
                   }
                   else { // active session
                      // first set the new date to now
                      this._loggedInUsers[x].lastSeen = now; // update the actual user data
                      sys.puts(this._loggedInUsers[x].lastSeen);
